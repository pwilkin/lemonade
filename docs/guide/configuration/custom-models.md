...
| `--recipe RECIPE` | Recipe to associate with the new `user.*` model. Common values: <!-- BEGIN GENERATED: recipe-values -->`llamacpp`, `whispercpp`, `moonshine`, `kokoro`, `sd-cpp`, `flm`, `ryzenai-llm`, `vllm`, `openmoss`, `thinksound`, `acestep`, `trellis`, `collection.omni`<!-- END GENERATED: recipe-values -->. |
